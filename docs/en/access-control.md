...
If you create a **subproject**, that subproject inherits the parent project’s visibility type and permission mode at creation time. You may also choose to copy the parent project’s members and group access during creation. This inheritance/copy happens **only when the subproject is created**: once created, the subproject is managed as an independent project, and later membership or permission changes do not automatically stay synchronized with the parent project.
