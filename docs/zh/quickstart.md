...
对于规模较大的研究任务，Airalogy 还支持在一个项目下继续创建 **一级子项目（Subproject）**。子项目仍然是完整的项目单元，拥有自己的成员、协议、设置页面和永久链接；因此您可以将一个大型研究按实验阶段、研究模块或协作分工拆分为多个子项目进行管理。当前版本中，子项目仅支持 **1 层层级**，不支持继续嵌套子项目。
...
* 一个默认实验室：
  ...
  * 在当前公开版本中，每个账号最多可拥有 **3 个实验室**，其中包含系统注册时自动创建的这个默认实验室
* 在该默认实验室下，一个默认项目：
...
如果您后续手动创建新的实验室，Airalogy 还会在该实验室下自动创建两个默认项目：

* `public_protocols`：用于向其他实验室分享协议的公开项目
* `lab_protocols`：用于实验室内部共享协议的私有项目
